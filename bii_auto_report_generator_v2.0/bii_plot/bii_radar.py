...
def graph_data(in_data, in_data_comp, code):
    ...
    if in_data_comp is None:
        ...
        [t, r, d, b, col, ra, cz, izc, score] = in_data_comp
        data = [
        ['Trust', 'Res', 'Div', 'Ment Str', 'Collab', 'Res All', 'Com Zone', 'In Zone'],
        (code, [
            [mean(trust), mean(res), mean(div), mean(bel), mean(collab), mean(resall), mean(comfort), mean(iz)],
            [mean(t), mean(r), mean(d), mean(b), mean(col), mean(ra), mean(cz), mean(izc)]])
        ]
    return data

def bii_radar(code, data_bii, data_comp, comp, wg, proj):
    score = np.mean(data_bii[8])

    if len(code) == 2 and not isinstance(code, basestring):
        code = code[0] + " " + code[1]

    data = graph_data(data_bii, data_comp, code)
    ...
    if data_comp is None:
        label = ['Score']
        lgd = plt.legend(label, loc=(0.82, .95), labelspacing=0.1)
    else:
        label = ['Score',comp]
        lgd = plt.legend(label, loc=(0.82, .95), labelspacing=0.1)
